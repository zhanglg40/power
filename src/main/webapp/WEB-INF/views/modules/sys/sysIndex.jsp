...
<head>
	...
	<style type="text/css">
		...
		#header {margin:0 0 0px;position:static;} #header li {font-size:14px;_font-size:12px;}
	...
	var video = document.getElementById("alertVoice");
	var jj=0;
	function showMod(){
		 $('#myModal').modal('show');
                	 ...
                	 $("#mlist").empty();
                     	...
                     	if(data.data!=null&&data.data.length!=0){
                     		var html="  <table class=\"table table-striped table-bordered table-condensed\"><tr>";
                     		 html=html+"<td>编号</td><td>设备名称</td><td>设备类型</td><td>报警时间</td><td>报警类型</td><td>处理</td></tr>";
                     		for(var i=0;i<data.data.length;i++) {
                     			 var str="<tr><td>"+(i+1)+"</td>";
                     			 if(typeof(data.data[i]['sbbName'])=="undefined"){
                                     str= str+"<td>-</td>"
                                 }else
                                 str= str+"<td>"+data.data[i]['sbbName']+"</td>";
                                 if(typeof(data.data[i]['sbbType'])=="undefined"){
                                     str= str+"<td>-</td>"
                                 }else
                                 str= str+"<td>"+data.data[i]['sbbType']+"</td>";
                                 if(typeof(data.data[i]['alertTime'])=="undefined"){
                                     str= str+"<td>-</td>"
                                 }else
                                 str= str+"<td>"+data.data[i]['alertTime']+"</td>";
                                 if(typeof(data.data[i]['alertType'])=="undefined"){
                                     str= str+"<td>-</td>"
                                 }else
                                 str= str+"<td>"+data.data[i]['alertType']+"</td>";
                                 str= str+"<td><a onclick=\"return delAlert('"+data.data[i]['id']+"')\">处理</a></td>";
                                 str= str+"</tr>";
                     		}
                     		html=html+str+"</table>";
                     		$("#mlist").html(html);
	    ...
	    function delAlert(id){
	    	 $.ajax({
                ...
                // dataType: "json",
                 async: false,
                 url : '${ctx}/power/alert/delData?id='+id,
		...
		$(document).ready(function() {
			//location.href='/power_cloud_manager/theme/default?url='+location.href;
			 video = document.getElementById("alertVoice");
				...
				if ($(menuId).length > 0){
					...
					$.get($(this).attr("data-href"), function(data){
						...
						// 默认选中第一个菜单
					 	$(menuId + " .accordion-body a:first i").click();
					 $(menuId + " .accordion-body li:first li:first a:first i").click();
					//	$('#mainFrame').attr("src","${ctx}/sys/index");
					});
				}
				// 大小宽度调整
				wSizeWidth();
	...
	<div id="main">
		<div id="header" class="navbar navbar-fixed-top">
			<div class="navbar-inner">
				<div class="brand" style="padding-bottom: 0px;padding-top: 3px"><div id="productName" style="font-size:21px">哈德森科技-智联网</div>
                    <div style="font-size: x-small;text-align: center">- 智慧用电云平台 -</div>
                </div>
				<ul id="userControl" class="nav pull-right">
					<li id="alert" style="display:none"><a onclick="return showMod()" title="访问网站主页"><img height="25" width="25" src="${ctxStatic}/img/alert.gif" alt="" /></a>
				    <audio id="alertVoice" style="display:none" src="${ctxStatic}/img/4031.mp3" controls="controls"></audio>
					</li>
					...
					<li id="themeSwitch" class="dropdown">
						...
						<ul class="dropdown-menu">
							...
							<li><a href="javascript:cookie('tabmode','0');location=location.href">${tabmode eq '1' ? '关闭' : '开启'}页签模式</a></li>
						</ul>
				...
				<div class="nav-collapse">
					<ul id="menu" class="nav" style="*white-space:nowrap;float:none;">
					   <!--          <li class="menu1">
										<a class="menu1" href="/power_cloud_manager/a/sys/index" data-id="b946814210454e32b2b2693349305798" target="mainFrame"><span>首页</span></a>
								</li> -->
	    ...
	    <div class="container-fluid">
			<div id="content" class="row-fluid">
				<div id="left" style="background:#293846"><%-- 
		...
		<div style="width: 800px;display:none" class="modal fade" id="myModal"  tabindex="-2" role="dialog" aria-labelledby="myModalLabel" >
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">数据报警列表</h4>
            </div>
